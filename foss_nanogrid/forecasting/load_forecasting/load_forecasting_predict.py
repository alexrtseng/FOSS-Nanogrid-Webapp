...
Model files have been saved in native XGBoost binary format in the load_models_v4 folder. Version 1 of the models is
...
class LoadPredict:
    ...
    def _load_models(self) -> list:
        ...
        for i in range(NUM_OF_MODELS):
            ...
            reg.load_model(
                f"forecasting/load_forecasting/load_models_versions/load_models_v4/xgboost_load_model_{i}.bin"
            )
            regs.append(reg)
        return regs
    ...
    def _get_weather_features(
        ...
        response_data = get_weather_data_batch(
            start=start,
            end=end,
    ...
    def forecast_load_timestamp_range(
        self,
        start: pd.Timestamp,
        end: pd.Timestamp,
        sm: SmartMeter,
        ...
        df_features = self._get_weather_features(
            start, end, resolution, min_resolution, sm
        )
        ...
        return pd.DataFrame({"datetime": datetime_col, "load_pred": predictions})

    # Take predictions data from (columns: datetime, load_pred) and creates json file in SolarCast format
    # **kwargs: sm - SmartMeter object, date_run - date the forecast model was run
    @staticmethod
    def forecasted_power_to_dict(
        _predictions: pd.DataFrame, model="XGBoost_load_v4", **kwargs
    ) -> dict:
        ...
        for _, row in predictions.iterrows():
            values.append({"Timestamp": row["datetime"], "kW": row["load_pred"]})
        ...
        if len(values) >= 1:
            forecast_dict = {
                ...
                "date_run": (
                    ...
                    if "date_run" not in kwargs
                    else kwargs["date_run"]
                ),
                "forecasted_dates": f"{values[0]['Timestamp']} to {values[-1]['Timestamp']}",
                "values": values,
            }
        else:
            forecast_dict = {"values": values}
