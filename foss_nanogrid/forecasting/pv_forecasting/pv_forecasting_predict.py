...
class PVPredict:
    ...
    def _load_models(self) -> list:
        ...
        for i in range(NUM_OF_MODELS):
            ...
            reg.load_model(
                f"forecasting/pv_forecasting/pv_models_v1/xgboost_pv_model_{i}.bin"
            )
            regs.append(reg)
        return regs
    ...
    def forecast_pv_timestamp_range(
        ...
        df_features = self._get_weather_features(
            start, end, resolution, min_resolution, pv
        )
        ...
        predictions = predictions * pv.capacity # in MW
        return pd.DataFrame({"datetime": datetime_col, "pv_pred": predictions})

    # Take predictions data from (columns: datetime, pv_pred) and creates json file in SolarCast format
    # **kwargs: pv - PVPanel object, date_run - date the forecast model was run
    @staticmethod
    def forecasted_power_to_dict(
        _predictions: pd.DataFrame, model="XGBoost_pv_v1", **kwargs
    ) -> dict:
        predictions = _predictions.sort_values(by="datetime").copy()
        values = []
        for index, row in predictions.iterrows():
            values.append({"Timestamp": row["datetime"], "MW": row["pv_pred"]})
        ...
        if "pv" in kwargs and len(values) >= 1:
            forecast_dict = {
                ...
                "model": model,
                "date_run": (
                    pd.Timestamp.now(tz="Asia/Nicosia")
